...
define([
  ...
  describe("Facet option destruction", function () {

    it('shouldn\'t leave html after destruction', function () {
      $facetOption.facet_option($.extend({}, configurationMock, {
        entry: optionContent,
        filter: filterMock,
      }));
    ...
    it('shouldn\'t leave data in facet-option data key after destruction',
      function () {
        $facetOption.facet_option($.extend({}, configurationMock, {
          entry: optionContent,
          filter: filterMock,
  ...
  describe("Facet option initialization", function() {
    ...
    it('should be stored at facet-option key', function() {
      expect(this.facetOption).not.toBe(undefined);
    });

    it('should have inactive state by default', function () {
      expect(this.facetOption.getState()).toBe(states.inactive);
      expect(this.facetOption.isActive()).toBe(false);
    });

    it('should call on_deactivated after applying default inactive state',
      function() {
    ...
    it('should call disable_expansion on object creation ' +
  ...
  describe('Facet option state changes', function () {
    beforeEach(function() {
    ...
    it('should have active proper state after activation/deactivation', function() {
    ...
    it('should treat partially active state as inactive - a child is active then', function() {
      this.facetOption._setState(states.partiallyActive);
      expect(this.facetOption.isActive()).toBe(false);
    });

    it('should call on_activated on activation', function() {
      this.facetOption.activate(states.limitTo);
    ...
    it('should call on_deactivated on deactivation', function() {
      this.facetOption.activate(states.limitTo);
      this.facetOption.deactivate(states.limitTo);
      expect(configurationMock.on_deactivated).toHaveBeenCalled();
    });

    it('should call update method of the filter cause it doesn\'t have parent to propagate', function() {
      this.facetOption.activate(states.limitTo);
  ...
  describe('Facet option events', function() {
    ...
    it('should have state exclude when activated with shift', function() {
      var shiftToggled = jQuery.Event(this.facetOption.options.toggle_filter_event);
      shiftToggled.shiftKey = true;
      this.facetOption.$toggle_filter_button.trigger(shiftToggled);
      expect(this.facetOption.getState()).toBe(states.exclude);
    });

    it('should have state limitTo when activated without shift', function() {
      var noShiftToggled = jQuery.Event(this.facetOption.options.toggle_filter_event);
      noShiftToggled.shiftKey = false;
      this.facetOption.$toggle_filter_button.trigger(noShiftToggled);
      expect(this.facetOption.getState()).toBe(states.limitTo);
    });

    it('should have state deactivated when toggled after the state was ' +
    ...
    it('triggers activated event on activated, and deactivated when deactivated', function() {
      var toggledEvent = this.facetOption.options.toggle_filter_event;
      var spyActivated = spyOnEvent($facetOption, this.facetOption.events.activated);
      var spyDeactivated = spyOnEvent($facetOption, this.facetOption.events.activated);
      this.facetOption.$toggle_filter_button.trigger(toggledEvent);
  ...
  describe('expandable option with initial state', function() {
    ...
    it('does not activate a parent option when child are not identical', function () {
      this.facetOption.deactivate();
      articles.activate('+');
      expect(this.facetOption.getState()).toBe(states.partiallyActive);

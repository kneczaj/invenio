...
define([
  'jquery',
  'js/search/facets/facet_options_list',
], function($) {
   ...
   * @class FacetFilter
   */
  function FacetFilter(element, options) {
  ...
  FacetFilter.prototype = {
     ...
     *
     * @param filter {FacetOption} option
     * @param action {states}
     * @private
     */
    ...
    loadState: function loadState(saved_state) {
      ...
      if (!this.is_loaded) {
        this.loadedPromise.done(function() {
          that._loadState(saved_state_copy);
        });
        return;
      }
    ...
    /**
     * Returns FacetOption object with given id from all of children in DOM.
     *
  ...
  $.fn.facet_filter = function (option) {
    ...
    return $elements.map(function (idx, element) {
      ...
      if (!object) {
        object = new FacetFilter($element, options);

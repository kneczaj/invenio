...
define([
   ...
   * @constructor
   */
  function FacetOption(element, options) {
  ...
  FacetOption.prototype = {

    events: FacetOption.events,
     ...
     *
     * @method getPath
     * @returns {Array} with ids of FacetOption objects
     */
    getPath: function() {
      if (!this.path) {
        if (!this.parent)
     ...
     *
     * There can be a couple of activation modes. A FacetOption is not active
    ...
    activate: function(new_state) {
      ...
      this.$entry.trigger($.Event(this.events.activated, {
    ...
    deactivate: function() {
      ...
      this.$entry.trigger($.Event(this.events.deactivated, {
        filter_option_id: this.id
      }));
    },
  ...
  $.fn.facet_option = function (option) {
    ...
    return $elements.map(function (idx, element) {
      ...
      if (!object) {
        object = new FacetOption($element, options);
  ...
  $.fn.facet_option.defaults = {
     ...
     *
     * FacetOption element should be accessible inside these functions by
     * $facet_option.data('facet-option')
     *
     * @param $facet_option jQuery selector to an element with FacetOption object
     *  built on top of it
     */

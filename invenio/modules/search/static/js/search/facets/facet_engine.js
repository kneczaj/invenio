...
define([
   ...
   * @constructor
   */
  function FacetEngine(element, options) {
    ...
    // array with all the filter gui elements
    this.filter_boxes = [];
  ...
  FacetEngine.prototype = {

    events: FacetEngine.events,
    ...
    init: function() {
      ...
      for (var i in facets) {
        ...
        $new_filter.facet_filter($.extend({}, this.options.filter_details, {
          url: f.url,
          header: f.title,
          id: facet_id,
          facet_engine: this,
        ...
        $new_filter.on($new_filter.data('facet-filter').events.updated, function(event) {
          that.$element.trigger(that.events.updated);
        });

        this.filter_boxes[facet_id] = $new_filter;
    ...
    _gatherResults: function(methodName) {
      var results = {};
      for (var i in this.filter_boxes) {
        var filter = this.filter_boxes[i].data('facet-filter');
    ...
    loadState: function(saved_state) {
      ...
      for (var box_id in saved_state) {
        var filter = this.filter_boxes[box_id].data('facet-filter');
  ...
  $.fn.facet = function (option) {
    ...
    return $elements.map(function (idx, element) {
      ...
      if (!object) {
        object = new FacetEngine($element, options);
